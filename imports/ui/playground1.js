import './playground1.html';
import './playground1.css';
...
Template.playground1.events({
  //switched of 'stardac' working on click (tracks are playing without On button)
    ...
    "click .js-controlRunner": function () {
        var c = Session.get('runner');
        var mach = MusicMachine.findOne({});

        if (c === 0){
            Session.set('runner', 1);
            MusicMachine.update({_id:mach._id}, {$set: {runner: 1}});
        } else {
            Session.set('runner', 0);
            MusicMachine.update({_id:mach._id}, {$set: {runner: 0}});
        }
    },

    "click .js-stopRunner": function() {
      var c = Session.get('runner');
      var mach = MusicMachine.findOne({});
      if (c === 1) {
      Session.set('runner', 0);
 ...
 Template.playground1.onRendered(function() {
  $('h2').hide();
